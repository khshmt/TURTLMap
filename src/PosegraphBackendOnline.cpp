...
namespace pose_graph_backend
{
    ...
    void PosegraphBackendOnline::callbackIMU(const sensor_msgs::ImuConstPtr &imu_msg)
    {
        ...
        else
        {
            ...
            // get IMU msg
            gtsam::Vector3 imu_acc_ = gtsam::Vector3(imu_acc.x(), imu_acc.y(), imu_acc.z());
            gtsam::Vector3 imu_gyro_ = gtsam::Vector3(imu_gyro.x(), imu_gyro.y(), imu_gyro.z());

            imu_latest_rot_ = imu_rot;

            // deal with the pim
            // compute dt_imu
            posegraph_->pim_->integrateMeasurement(imu_acc_,
                                                   imu_gyro_,
                                                   posegraph_->params_->imu_params_.dt_imu);
            pim_dvl_->integrateMeasurement(imu_acc_,
                                           imu_gyro_,
                                           posegraph_->params_->imu_params_.dt_imu);
            imu_count_++;
